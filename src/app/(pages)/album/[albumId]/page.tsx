...
const Albums = ({ params }: { params: { albumId: string } }) => {
  ...
  const { addAlbum, removeAlbum, albums } = useMusicPlayer();
  const isAlbum = albums.some((album) => album.id === albumId);
        ...
        <div className="container mx-auto flex px-5 py-10 md:flex-row flex-col items-center">
          ...
          <div className="lg:flex-grow md:w-1/2 lg:pl-24 md:pl-16 flex flex-col md:items-start md:text-left items-center text-center">
            ...
            <Button
              ...
              onClick={() =>
                isAlbum ? removeAlbum(`${album?.id}`) : addAlbum(album)
              }
            >
              {isAlbum ? 'Remove Album' : 'Save Album'}
            </Button>
          </div>
        </div>
      </section>
      ...
      <div>
        ...
        <ArtistSlider>
          <div className="flex items-center justify-between gap-3 text-center">
            {album?.artists?.all?.map((artist: any) => (
              <div
                key={artist.id}
